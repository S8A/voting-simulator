...
def make_table(widths, header, body):
    ...
    for i, row in enumerate(table):
        ...
        rtable.append(sep.join([f'{tw.shorten(str(content), w):<{w}}'
                                  for content, w in wrow]))
    return '\n'.join(rtable)

def make_pattern_map_table(pattern_map):
    """Converts voting pattern map into a text table."""
    widths = [40, 20, 10]
    header = ['Voting pattern', 'Voters', 'Percent']
    body = [(pattern, *popularity)
            for pattern, popularity in sort_dict_desc(pattern_map)]
    return make_table(widths, header, body)

def make_results_table(results, count_type='Votes', percent_column=True):
    """Sorts results and converts them into a table."""
    widths = [20, 20]
    header = ['Candidate', count_type]
    total = sum(list(results.values()))
    sorted_results = sort_dict_desc(results)
...
def print_pattern_map(pattern_map):
    print(make_pattern_map_table(pattern_map))
    print()

def print_results(normalized_results):
    print(make_results_table(normalized_results))
    print()
...
class VotingPattern:
    ...
    def __init__(self, sequence, name=None):
        """Creates a voting pattern object from the given preference order."""
...
class Candidate:
    ...
    def __init__(self, party, number):
        if party not in PARTIES.keys():
            raise Exception('Non-existing party')
        self.party = party
        self.number = number

    def __repr__(self):
        return f'{PARTIES[self.party]} {self.number}'
...
class VotingResults:
    ...
    def __init__(self, name, results, winners, count_type='Votes',
        ...
        Args:
            ...
            results: Normalized results (dict Candidate->(int, float))
    ...
    def results_table(self):
        """Returns the voting results table."""
        return make_results_table(self.results,
                                  count_type=self.count_type,
                                  percent_column=self.percent_column)
